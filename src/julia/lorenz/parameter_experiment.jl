...
N_data = 100000
lambda = 0.01
sigma_perturb_vec = 10 .+ 5 .* randn(1, N_data)
...
pTraj=plot(1:nT+1, trajectories[1:30,:]', title= string(nT) * " " * L"\theta" * " trajectories", 
                label=false, xlabel="t [seconds]", ylabel=L"\theta(t)") 

# set train args and train
epochs=10
args = train_Args(Int(floor(0.8 * N_data)), Int(floor(0.2 * N_data)), 10000, 3e-4, 10, epochs, gpu)
train_data, test_data, ŷ_vec_train, ŷ_vec_test, train_acc_vec, test_acc_vec = train(trajectories, , args, lambda)
...
# plot(x, abs(ŷ_vec_train - y_vec_train))
